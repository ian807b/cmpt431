...
void compute_page_rank(uintV start, uintV end, PageRankType pr_curr[], PageRankType pr_next[],
  ...
  for (int iter = 0; iter < max_iters; iter++) {
    for (uintV v = start; v < end; v++) {                    // For all v in V
      ...
      for (uintE i = 0; i < in_degree; i++) {                // For all incoming edges to V
        ...
        if (u_out_degree > 0)
          pr_next[v] += (pr_curr[u] / (PageRankType)u_out_degree);  // Summation of PR(u)/d(u)
      }
    }
    barrier.wait();
...
void pageRankParallel(Graph &g, uint max_iters, uint n_threads) {
  uintV n = g.n_;
  ...
  for (uintV i = 0; i < n_threads; i++) {
    ...
    threads.emplace_back(compute_page_rank, start, end, pr_curr, pr_next, std::ref(barrier),
                         max_iters, std::ref(g), std::ref(time_per_thread), i);
  }
