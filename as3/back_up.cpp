...
void compute_page_rank(uintV start, uintV end, PageRankType pr_curr[],
                       std::atomic<PageRankType> *pr_next, CustomBarrier &barrier, uint max_iters,
                       Graph &g, std::vector<double> &time_per_thread, uintV thread_id) {
  timer timer;
  timer.start();
  for (int iter = 0; iter < max_iters; iter++) {
    ...
    if (thread_id == 0) {
      for (uintV v = 0; v < g.n_; v++) {
        pr_next[v].store(PAGE_RANK(pr_next[v].load(std::memory_order_relaxed)),
                         std::memory_order_relaxed);
        pr_curr[v] = pr_next[v].load(std::memory_order_relaxed);
        pr_next[v].store(0.0, std::memory_order_relaxed);
      }
    }
    barrier.wait();
  }
